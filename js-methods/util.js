...
function getUrlParam() {
    ...
    var query = decodeURIComponent ? decodeURIComponent(location.search.substring(1)) : location.search.substring(1)
...
function setUrlByParam(param) {
    ...
    return url == "" ? url : getHost() + location.pathname + '?' + url.substring(1)
}
        ...
        if (this.hasLocalStorage()) {
            ...
            try {
                ...
                for (var i = 0; i < ca.length; i++) {
                    var temp = ca[i].split('=');
                    if (temp[0].toLowerCase().replace(/ /ig, '') == key.toLowerCase()) {
                        value = temp[1]
                        break;
                    }
                }
